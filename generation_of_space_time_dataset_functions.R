...
#[12] "gen_ar1_ts_fun" # rednoise generation given an AR1 coefficient, starting value
#[13] write_out_raster_fun ## Utility function to quickly write out a stack or brick of rasterlayer to disk using names of layers
...
##### Functions used in the script: "generation_of_space_time_dataset_11152013v6.R" #####
...
generate_raster_region <-function(j,list_param){
  ...
  NA_flag<-list_param$NA_flag_val
  seed_nb<-list_param$seed_nb[j] #should contain seed number for every random image...
...
### Make this more efficient later on!!! this is ok for small images for  now...
convert_multiple_ts_to_raster <-function(list_temp_pattern,r_stack,out_suffix,NA_flag_val,file_format,out_dir,operation="addition"){
  #modify later for efficiency if needed, this is a small image!!!
  ...
  temp_pattern <- do.call(cbind,list_temp_pattern) 
  names(r_list) <- paste("ts",1:nlayers(r_stack),sep="_") #use names in new raster package
...
gen_ar1_ts_fun <-function(i,t0_val,ar1_coef,nt,sd_rd=1,mean_rd=0,seed_nb){
  ts_val <- vector("numeric",length=nt)
  set.seed(seed_nb[i])
  rd_val <- rnorm(nt,sd=sd_rd,mean=mean_rd)
  ts_val[1] <- t0_val[i]
  for(j in 2:nt){
    ts_val[j] <- ts_val[j-1]*ar1_coef + rd_val[j] 
  } 
  return(ts_val)
}
