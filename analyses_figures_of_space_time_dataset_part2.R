...
tmp_names<-c(paste("PC",1:npc,sep="")) #PCA with varimax rotation
mode_list<- tmp_names
out_prefix_n<-paste("PC_L12_cor_Tel",out_suffix,sep="")
...
out_prefix_n<-paste("PC_L12_quadratures",out_suffix,sep="")
pc_quad_comp_obj<- crosscor_lag_analysis_fun(mode_list,mode_list,d_z2,lag_window,fig=F,out_prefix_n)

#### Now do analyses for L=24
...
x<- x[1:(nrow(x)-6),]
x2<- df[1:(nrow(df)-1),]
dat3<- cbind(x,df[1:(nrow(df)-1),])
d_ts <- ts(data=dat3)
colnames(d_ts)
rownames(d_ts)
d_z3<-as.zoo(d_ts)  #### THIS IS THE TIME SERIES OBJECT USED LATER ON

lag_window <- 12
mode_list<-tmp_names<-c(paste("PC",1:npc,sep="")) #PCA with varimax rotation
out_prefix_n<-paste("PC_L24_cor_Tel",out_suffix,sep="")
pc_L24_obj<-crosscor_lag_analysis_fun(telind,mode_list,d_z3,lag_window,fig=F,out_prefix_n)
out_prefix_n<-paste("PC_L24_quadratures",out_suffix,sep="")
pc_quad_comp_obj<- crosscor_lag_analysis_fun(mode_list,mode_list,d_z3,lag_window,fig=F,out_prefix_n)
