...
list_temp_pattern_produced <- vector("list",length=10)
...
#Prepare to generate images combining time and space
out_suffix_s <- paste("r_ts7_",out_suffix,sep="")
operation<-"multiplication"
r_ts7 <- combine_space_time_structure(temp_pattern,r_stack,out_suffix_s,NA_flag_val,file_format,operation)

levelplot(r_ts7,layers=c(1,78,156,234,312))
levelplot(r_ts7,layers=c(1,178))
levelplot(r_ts7,layers=1:48)
...
b<-0
x
A_seq <-sine_structure_fun(1:period,period/2,0,1,0) #different weight with negative!!!
plot(A_seq)
#if A_seq provided use the sequence
...
levelplot(r_ts8,layers=c(1,78,156,234,312))
levelplot(r_ts8,layers=c(1,178))
levelplot(r_ts8,layers=1:48)
dev.
list_temp_pattern_produced[[8]] <- temp_pattern
names(list_temp_pattern_produced)[8] <- out_suffix_s
plot_time_space_pattern(temp_pattern,r,out_suffix_s)
#dev.set(2) ##reactivate previous plotting window device
#par(mfrow=c(1,1))
...
levelplot(r_ts9,layers=c(1,78,156,234,312))
levelplot(r_ts9,layers=c(1,178))
#13,73,157,240
levelplot(r_ts9,layers=70:130)
...
levelplot(r_ts10,layers=c(1,78,156,234,312))
levelplot(r_ts10,layers=c(1,178))
#13,73,157,240
levelplot(r_ts10,layers=70:140)

list_temp_pattern_produced[[10]] <- temp_pattern
names(list_temp_pattern_produced)[10] <- out_suffix_s
plot_time_space_pattern(temp_pattern,r,out_suffix_s)

#### create rgf and stack...for all created time series files...

#example for one series:
out_suffix_s <- paste("r_ts1_",out_suffix,sep="")
out_prefix_str <- "test_12c_12r_"
list_rgf <- create_idrisi_rgf(file_pat="",out_suffix_s,in_dir,out_prefix=out_prefix_str)
list_r_ts <- create_raster_list_from_file_pat(file_pat="",out_suffix_s,in_dir=".",out_prefix="",file_format=".rst")

#Create rgf and stack for all series...
list_out_suffix_s <- paste("r_ts",1:10,"_",out_suffix,sep="")
lapply(list_out_suffix_s,FUN=create_idrisi_rgf,in_dir=in_dir,out_prefix=out_prefix_str) 

list_raster_ts <-lapply(list_out_suffix_s,FUN=create_raster_list_from_file_pat,in_dir=in_dir) 
